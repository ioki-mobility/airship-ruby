...
RSpec.describe Airship::Api::NamedUserTagsUpdate do
  ...
  let(:add_tags) { %w[vip poweruser] }
  let(:remove_tags) { ['demo_user'] }

  let(:expected_endpoint) { 'named_users/tags' }
  ...
  context 'with empty "add_tags" parameter' do
    let(:add_tags) { nil }
  ...
  context 'with empty "remove_tags" parameter' do
    let(:remove_tags) { nil }
